                ...
                if clusters.index(cluster) != clusters.index(win):
                    win_distances[clusters.index(cluster)] = \
                        sklearn.metrics.pairwise.euclidean_distances(win.centroid,
                                                                     cluster.centroid)
            sortedWinDistances = sorted(win_distances.items(), key=operator.itemgetter(1))
            if len(sortedWinDistances) <= 2:
                radius = sortedWinDistances[self.minPts - 2][1]
            else:
                radius = sortedWinDistances[self.minPts - 1][1]
            win.set_radius(radius)
            win_neighbors = []
            for d in sortedWinDistances:
                if d[1] <= radius:
    ...
    def update_cluster(self, win, input_vector, win_neighbors):
        ...
        for n in win_neighbors:
            ...
            power = -(sklearn.metrics.pairwise.euclidean_distances(last_centroid, win.centroid)) / (
                    2 * (win.radius ** 2))
            beta = math.exp(power)
            n.centroid = np.add(np.asfarray(last_centroid, float),
                                self.alpha * beta * np.subtract(np.asfarray(win.centroid, float),
                                                                np.asfarray(last_centroid, float)))
